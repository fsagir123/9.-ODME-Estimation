...
new_directory = 'C:/Users/fsagir/Desktop/Work/10. Python & ML/2. Projects/11. ODME to Turning Volumes/Elm St'  # Replace with your desired path
...
#Reading layout
df=pd.read_csv('LAYOUT_Child.csv',skiprows = 1)
df.index = df['INTID']
...
df=pd.read_csv('VOLUME_Child.csv',skiprows = 2)
df.index = df['INTID']
df.fillna(0, inplace=True)
...
#Reading modified OD
modified_OD = pd.read_csv('Parent_OD_matrix_for_one_way_outer_rd_with_u_turn.csv',index_col=0)
modified_OD = modified_OD.iloc[:15, :15]
print(modified_OD)
modified_OD = modified_OD.values.flatten()
modified_OD = np.array(modified_OD)
...
print(origin)
volume_from_code = np.dot(A_whole,modified_OD.T)
...
volume_for_synchro = volume_deviation.pivot(index='Node Number', columns='Turning Movement Direction', values='Turning volume from code')

# Rename the index to 'INTID'
volume_for_synchro.index.name = 'INTID'

volume_file =  "Volume_for_Synchro_Child.csv"
volume_for_synchro.to_csv(volume_file,float_format='%.0f')

# Prepend "60 minute counts" to the CSV file
with open(volume_file, 'r') as f:
    content = f.read()

with open(volume_file, 'w') as f:
    f.write("60 minute counts\n" + content)

print(f"DataFrame written to {volume_file} with '60 minute counts' prepended.")